/*
  ...
  Note: if an image that has annotations is overwritten by a new version, only displays the
  annotations if ratio of the top image matches the stored annotations image ratio. To recover
  ...
  Author: [[w:User:Lupo]], June 2009 - March 2010
	...
	( function () { // Local scope
		...
		ImageAnnotation.prototype = {
			...
			initialize: function ( node, viewer, id ) {
				...
				if ( LAPI.DOM.hasClass( node, IA.annotation_class ) ) {
					var w_factor = viewer.full_img.width / IA.getIntItem( 'full_width_' + id, viewer.scope );
					var h_factor = viewer.full_img.height / IA.getIntItem( 'full_height_' + id, viewer.scope );
					if ( Math.abs( w_factor - h_factor ) > .01 ) { // This can be relaxed maybe
						throw new Error( 
							'Annotation ' + id + ' is rejected because ' +
							'it had incompatible full width/height annotation ' +
							'with current version of the file'
						);
					}
					// Extract the info we need
					var x = IA.getIntItem( 'view_x_' + id, viewer.scope ) * w_factor;
					var y = IA.getIntItem( 'view_y_' + id, viewer.scope ) * h_factor;
					var w = IA.getIntItem( 'view_w_' + id, viewer.scope ) * w_factor;
					var h = IA.getIntItem( 'view_h_' + id, viewer.scope ) * h_factor;
			...
			save: function ( editor ) {
				...
				LAPI.Ajax.editPage(
					...
					function ( request, ex ) {
						...
						if ( lk && lk.length && lk[ 0 ].firstChild && lk[ 0 ].firstChild.nodeName && lk[ 0 ].firstChild.nodeName.toLowerCase() === 'a' ) {
							lk = lk[ 0 ].firstChild;
		...
		var IA = {
			...
			tooltip_styles: { // The style for all our tooltips
				...
				color: 'inherit',
				backgroundColor: 'var(--image-annotator-background, #ffffe0)',
				padding: '0.3em',
			...
			createHelpLink: function () {
				...
				var anchor = $( 'a', msg )[0] || {};
				var title = anchor.textContent || 'Help about image annotations';
				var href = anchor.href || mw.util.getUrl( 'Help:Image-Annotator' );
				return $( '<a>', {
					title: title,
					target: '_blank',
					href: href,
					...
					$( '<img>', {
						alt: title,
						src: 'https://upload.wikimedia.org/wikipedia/commons/b/bb/Echo_question.svg',
						style: 'width: 16px; height: 16px; ' + (
							document.documentElement.dir === 'rtl' && /* no need to flip for these languages */['he', 'yi'].indexOf(document.documentElement.lang) === -1
								? 'transform: scaleX(-1);'
								: ''
						)
					} )
