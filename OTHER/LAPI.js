...
if ( window.LAPI_file_store === undefined ) {
	var LAPI_file_store = '(https?:)?//static\\.wikitide\\.net/';
}
...
// DEPRECATED: Use built-in String.prototype.includes() instead
	...
	LAPI.WP = {
		...
		revisionFromHtml: function ( htmlOfPage ) {
			...
			if ( window.RLCONF ) { // MW 1.32+
				revision_id = htmlOfPage.match( /RLCONF=\{[\s\S]*"wgCurRevisionId"\s*:\s*(\d+),/ );
...
if ( !LAPI.Ajax.doAction ) {
	...
	LAPI.Ajax.editPage = function ( page, modify, failure ) {
		LAPI.Ajax.doAction( page, 'submit', 'editform', modify, failure );
