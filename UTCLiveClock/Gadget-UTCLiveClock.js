...
function liveClock() {
	...
	var location = window.UTCLiveClockLocation || 'inside';
	var showOutsideDropdown = location === 'outside';
...
function addLiveClockInstance( clockLocation ) {
	...
	$( node ).on( 'click', function ( e ) {
		new mw.Api({userAgent: 'Gadget-UTCLiveClock/2025'}).post( { action: 'purge', titles: mw.config.get( 'wgPageName' ) } ).then( function () {
			location.reload();
		}, function () {
